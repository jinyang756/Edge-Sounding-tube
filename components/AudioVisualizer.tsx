...
const AudioVisualizer: React.FC<AudioVisualizerProps> = ({ stream, isRecording }) => {
  ...
  useEffect(() => {
    ...
    // Critical: Resume context if suspended (common in Chrome/Edge)
    ...
    if (!analyserRef.current) {
        ...
        analyserRef.current.smoothingTimeConstant = 0.5; // Make it snappier
    }
    ...
    try {
        ...
        console.error("Error connecting stream to visualizer", err);
    }
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Boost sensitivity: Multiply by 1.2 to make bars taller for quieter sounds
        barHeight = (dataArray[i] / 255) * height * 1.2; 

        // Gradient color based on height
        const gradient = ctx.createLinearGradient(0, height, 0, height - barHeight);
        gradient.addColorStop(0, '#e0e7ff'); // Light indigo
        gradient.addColorStop(1, '#6366f1'); // Indigo-500 (Darker for visibility)
        ...
        ctx.beginPath();
        if (ctx.roundRect) {
            ctx.roundRect(x, height - 10 - barHeight, barWidth, barHeight, 2); 
        } else {
      ...
      // 2. Calculate Average Volume (RMS) for the bottom bar
      let sum = 0;
      for(let i = 0; i < bufferLength; i++) {
        sum += dataArray[i];
      }
      const average = sum / bufferLength;
      // Boost volume sensitivity for the indicator
      const volumePercent = Math.min((average / 100) * 1.5, 1); 
      ...
      ctx.fillRect(0, height - volumeBarHeight, width, volumeBarHeight);
    ...
    return () => {
      ...
      // Note: We don't disconnect sourceRef here to avoid cutting off audio if this unmounts/remounts rapidly,
      // but strict cleanup usually requires it. For now, we leave it to the parent stream cleanup.
    };
  }, [stream, isRecording]);
